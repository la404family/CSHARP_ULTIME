...
% Fichier : src/P1_04_a.tex
% Sujet   : Chapitre 4 : Structures conditionnelles
% ==============================================================================

\newpage
\section{Structures Conditionnelles}
\marginpar{\tiny\color{gray}P1\_04\_a.tex}
...
Dans sa forme traditionnelle, chaque section \texttt{case} non vide doit obligatoirement se terminer par une instruction de saut (\texttt{break}, \texttt{return}, \texttt{goto} ou \texttt{throw}). Contrairement à d'autres langages comme le C ou Java, C\# interdit la chute implicite (\emph{fall-through}) d'un cas exécutable vers le suivant : omettre \texttt{break} à la fin d'un cas contenant du code déclenche une erreur de compilation bloquante (\texttt{CS0163: Control cannot fall through from one case label to another}).
...
switch (codeErreurHttp)
{
    case 200:
        ...
        break;
    case 401:
    case 403: // Regroupement d'étiquettes (cases vides consécutifs partageant le même bloc)
