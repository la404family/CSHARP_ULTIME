...
\begin{itemize}
    ...
    \item \textbf{Erreur de borne (\textit{Off-by-one error})} : Utiliser \texttt{<=} au lieu de \texttt{<} lors du parcours d'un tableau classique avec une boucle \texttt{for}. Puisque les tableaux sont indexés à partir de 0, cela mène à une \texttt{IndexOutOfRangeException}.
\end{itemize}
\end{tcolorbox}
