...
\subsubsection{Comparatif et Critères de Choix : Pourquoi utiliser l'un ou l'autre ?}
...
\begin{center}
    \textbf{Tableau : Comparatif technique entre Visual Studio et Visual Studio Code} \\[0.3cm]
    \begin{tabular}{@{}lp{6.2cm}p{6.2cm}@{}}
        \toprule
        \textbf{Critère} & \textbf{Visual Studio (1.1.1)} & \textbf{Visual Studio Code (1.1.2)} \\
        \midrule
        \textbf{Nature} & IDE complet tout-en-un & Éditeur de code extensible \\
        \textbf{Systèmes d'exploitation} & Windows uniquement & Windows, macOS, Linux \\
        \textbf{Outillage intégré} & Concepteurs graphiques (WPF, WinForms), profiler mémoire natif & CLI .NET (\texttt{dotnet}), suite d'extensions (C\# Dev Kit) \\
        \textbf{Consommation système} & Empreinte mémoire et disque importante & Ultra-léger, démarrage rapide \\
        \textbf{Projets préconisés} & Monolithes d'entreprise, applications desktop Windows & APIs REST ASP.NET Core, microservices, projets multiplateformes \\
        \bottomrule
    \end{tabular}
\end{center}

\paragraph{Pourquoi choisir Visual Studio (1.1.1) ?}
Visual Studio est privilégié pour le développement de grandes solutions d'entreprise sous Windows. Il offre une intégration « tout-en-un » sans configuration complexe :
\begin{itemize}
    \item \textbf{Concepteurs visuels} : Édition WYSIWYG et concepteurs XAML natifs pour les applications bureau (WPF, Windows Forms).
    \item \textbf{Diagnostic avancé} : Outils de profilage mémoire/CPU au runtime, analyse de couverture de code et débogage pas-à-pas multi-threads.
    \item \textbf{Outillage de base de données} : Explorateur d'objets SQL Server (\emph{SSDT}) directement intégré dans l'IDE.
\end{itemize}

\paragraph{Pourquoi choisir Visual Studio Code (1.1.2) ?}
Visual Studio Code est l'outil idéal pour le développement web moderne, les architectures distribuées et les environnements multiplateformes :
\begin{itemize}
    \item \textbf{Multiplateforme natif} : Environnement de travail identique sous macOS, Linux et Windows.
    \item \textbf{Écosystème Web \& Microservices} : Parfaitement adapté au développement d'APIs REST ASP.NET Core associées à des applications Web single-page (Angular, React).
    \item \textbf{Conteneurs et CLI} : Exploite la CLI .NET (\texttt{dotnet}) et s'intègre naturellement avec Docker et les \emph{Dev Containers}.
\end{itemize}
...
\subsubsection{Les Composants Clés de la Chaîne d'Exécution .NET}

Voici le rôle exact de chaque composant principal dans le cycle de vie de votre code :

\begin{itemize}
    \item \textbf{La CLI (L'Infrastructure)} : Le cahier des charges standardisé qui dicte les règles d'interopérabilité et de structure de code entre langages .NET.
    \item \textbf{Roslyn (Le Compilateur C\#)} : L'outil de compilation statique qui analyse le code C\# (\texttt{.cs}) et le traduit en \textbf{Code IL} (bytecode intermédiaire).
    \item \textbf{Le CLR (Le Moteur d'Exécution Runtime)} : L'environnement d'exécution principal qui orchestre le programme et intègre ses sous-composants essentiels :
    \begin{itemize}
        \item \textbf{Le JIT (Just-In-Time Compiler)} : Sous-composant du CLR qui traduit à la volée l'IL en instructions binaires natives optimisées pour le processeur hôte (Intel, AMD, ARM).
        \item \textbf{Le Garbage Collector (GC)} : Sous-composant du CLR assurant la gestion et le recyclage automatique de la mémoire.
    \end{itemize}
\end{itemize}
...
namespace LesBases.Chapitre1; // C# 10 : File-scoped namespace (pas d'accolades)
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Le compilateur \textbf{JIT} traduit les blocs IL nécessaires en instructions machines natives.
...
\begin{center}
    ...
    \begin{tabular}{@{}lp{10cm}@{}}
        ...
        \textbf{Main()} & Point d'entrée de toute application console, explicite ou généré automatiquement par le compilateur (Top-Level Statements). \\
        \bottomrule
    \end{tabular}
\end{center}
