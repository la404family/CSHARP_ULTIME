...
\begin{tcolorbox}[colback=white,colframe=stringcolor,title=\textbf{Règles strictes de développement C\#}]
    \begin{itemize}
        \item \textbf{Configuration du Projet (\texttt{<Nullable>enable</Nullable>})} : L'annotation de référence nullable (\texttt{string?}) nécessite l'activation de l'analyse statique dans le fichier de projet \texttt{.csproj} (\texttt{<Nullable>enable</Nullable>}). Sans cette directive, les annotations \texttt{?} sur les types référence sont ignorées par le compilateur.
