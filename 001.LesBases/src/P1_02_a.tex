...
\begin{tcolorbox}[colback=backcolour,colframe=keywordcolor,title=\textbf{Règle : \texttt{const} vs \texttt{readonly}}]
    \begin{itemize}
        \item \textbf{\texttt{const}} : Pour les valeurs universelles connues avant l'exécution (Pi, nombre maximal, codes d'erreur HTTP). Implicitement \texttt{static}.
        \item \textbf{\texttt{readonly}} : Pour les valeurs calculées au démarrage ou injectées par le constructeur (identifiant de session, horodatage de création, configuration chargée depuis un fichier). Non statique par défaut.
    \end{itemize}
\end{tcolorbox}
...
\begin{itemize}
    \item \texttt{string code = Console.ReadLine();} générera un avertissement du compilateur, car la console peut renvoyer du vide.
