...
% Fichier : src/P1_12_a.tex
% Sujet   : Chapitre 12 : POO — Interfaces
% ==============================================================================

\newpage
\section{POO — Interfaces}
\marginpar{\tiny\color{gray}P1\_12\_a.tex}
...
// Note prod : En architecture réelle, un service d'E/S (Redis/Bdd) implémenterait IDisposable/IAsyncDisposable,
// accepterait des CancellationToken et s'appuierait sur ConcurrentDictionary pour la sécurité multi-thread.
...
// Règle C# moderne (.NET 8+) : Utiliser 'readonly struct' pour éliminer les copies défensives lors du tri
public readonly struct FinancialTransaction : IComparable<FinancialTransaction>
{
    public string TransactionId { get; init; }
    public decimal Amount { get; init; }

    // 5. Contrat de tri : doit retourner 0 (égal), < 0 (avant) ou > 0 (après)
    public readonly int CompareTo(FinancialTransaction other)
    {
        // Pas de vérification de nullité : un struct (type valeur) ne peut jamais être null
...
class Program
{
    static void Main()
    {
        ...
        // Note sur le JSON et l'interpolation : Dans une chaîne interpolée $"...", 
        // les accolades d'un JSON littéral doivent être doublées {{ "auth": true }},
        // ou enveloppées dans un Raw String Literal C# 11+ ("""{ "auth": true }""")
        ...
        // 8. Sort() inspecte la classe, trouve IComparable<T> et déclenche CompareTo() sans boxing grâce au readonly struct
        transactions.Sort(); 
...
\begin{itemize}
    ...
    \item \textbf{Ligne 39} : \texttt{FinancialTransaction} est déclarée en \texttt{readonly struct} et implémente \texttt{IComparable<T>}. L'immuabilité garantit qu'aucune copie défensive inutile n'est effectuée par le runtime lors de l'exécution de \texttt{Sort()}.
